...
- ONNX runtime as external `.so`/`.dylib` (bge-m3 int8, 1024-dim embeddings + NER); downloaded/verified per `onnx.yaml`. Bindings: `ort` **2.0.0-rc.13** (`load-dynamic` only, default features off) — a deliberate pre-release pin (ADR 0002); all API churn is isolated in `crates/embedding`

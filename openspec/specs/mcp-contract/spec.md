...
The entity payload returned by `get_entity_dossier` (the dossier entity), `catalog_entities` (entity entries), and `search` (entities attached to result chunks) SHALL include an additive `aliases` field: an array of strings, empty when the entity has no aliases, listing every surface name that has resolved to the entity (the entity's own name is not repeated in the list). Parameter schemas are unchanged by this addition. Recorded fixtures covering these tools are re-recorded to include the field.

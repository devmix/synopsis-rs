...
| `ingestion/src/runner/mod.rs` (13) | 1: `detect_source_type_matches_the_oracle_cases` (:1058) | 0 test fns — but the shared fixtures (`TestSource`/`MockEmbedding`/`MemoryIndex`/`Harness`/`TempDir`/`source_config`/`TEMP_COUNTER`) STAY inline in a reduced `#[cfg(test)]` block: they are imported by the 6 out-of-scope `cleanup.rs` tests (cleanup.rs:216) and are `#[cfg(test)]` mocks that can't move to `test_support` (Option A, task 1.12 Revision 1); moved tests get local copies | 12: via `walk_matched_files` |

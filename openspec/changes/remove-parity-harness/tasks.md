...
- [x] 1.1 Delete the crate + workspace member
...
**Cargo.lock.** Running any cargo command after the member is removed prunes the
`[[package]] name = "parity-harness"` entry. That lock-file change is **expected
and in-scope** (design D3). Do not hand-edit the lock.
...
5. `cargo test --workspace` green (no crate depends on parity-harness).

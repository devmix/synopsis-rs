...
- **WHEN** a fresh knowledge database is built from the init migration plus the search-text migration
- **THEN** the `chunks` table has a `search_text` column, the `chunks_fts` index indexes `search_text`, and `PRAGMA user_version` = 5

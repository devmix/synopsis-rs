...
- **WHEN** e2e-сценарии оракула (ingest → повторный ingest → изменение → удаление → prune → cleanup) прогоняются на Rust-конвейере
- **THEN** состояние БД и статистика соответствуют зафиксированным в оракуле результатам

...
**Примечание (решение человека 2026-08-22):** CEL-обёртка остаётся в крейте graph — единственный потребитель (линкеры, задача 1.9) живёт здесь же; резолвер сущностей ingestion CEL не использует (проверено по оракулу). Кандидат на выделение в отдельный крейт при появлении второго потребителя — перенос механический (модуль самодостаточен).

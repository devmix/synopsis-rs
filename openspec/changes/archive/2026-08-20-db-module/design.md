...
Batch-операции (GetByIDs, LinkBatch, DeleteByIDs) используют плейсхолдеры с батчами ≤ 500 строк (LinkBatch — 500×2=1000 параметров; GetByIDs — разбиение на чанки по 500). Лимит задокументирован в коде; тест на границе батча. **Пересмотрено 2026-08-20 (решение человека):** константы `ID_BATCH_SIZE`/`LINK_BATCH_SIZE` вынесены в крейт `config` (модуль `config::db`) — единый источник истины вместо 7 приватных копий в модулях db. **Изменение графа зависимостей:** db → config (config — лёгкий крейт без обратных зависимостей; решение человека 2026-08-20).

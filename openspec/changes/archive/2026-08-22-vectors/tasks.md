...
- [x] 1.4 Движок: каскадное удаление, перечисление, персистентность, пересоздание
  - **Ревизия 1 (2026-08-21, фидбек человека на ревью):** `VectorsError` вынести из `lib.rs` в отдельный файл `src/error.rs` — как в остальных крейтах workspace (например, `crates/embedding/src/error.rs`); ре-экспорт из корня сохранить (`pub use error::VectorsError` или эквивалент), доки и варианты не менять по сути.
...
- [x] 1.6 Интеграционные гейты: recall@10 и латентность на синтетике
...
- [x] 1.8 Полный бенчмарк N=1M + повтор на реальной фикстуре + appendix к ADR
  - **Ревизия 1 (2026-08-21, решение человека по итогам ревью 1.6):** перед бенчмарком устранить предсуществующий флейк юнит-теста `engine::tests::create_insert_build_search_roundtrip` (падает ~1/15 прогонов: равномерно-случайная геометрия 2500 векторов + требование точного top-1 vs вероятностный HNSW при неудачной инициализации k-means lance) — заменить генерацию на кластеризованную геометрию, чтобы top-1 был структурно гарантирован; тест-only правка в `crates/vectors/src/engine.rs`, стабильность подтвердить многократным прогоном.

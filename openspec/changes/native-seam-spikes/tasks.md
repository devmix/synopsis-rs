...
- [x] ~~3.1 Spike S3a: измерения usearch на синтетической фикстуре 250K×1024~~ — **ОТМЕНЕНА** (решение человека 2026-08-18, см. ревизию 1)
  - **Цель:** ~~воспроизводимые метрики usearch HNSW при RAM-бюджете ноутбука~~.
  - **Scope файлов:** ~~`crates/spikes/src/bin/s3a_usearch.rs` + зависимость usearch~~ — НЕ выполняется; usearch-наработки откачены (Cargo.toml/lock, spikes/Cargo.toml, s3a_usearch.rs возвращён к заглушке 0.2).
  - **Зависимости:** 0.2 (БД-фикстура не нужна — design D2).
  - **Критерии приёмки:** ~~таблица метрик; воспроизводимость двух прогонов (<5% разброса); результаты в `docs/adr/spike-s3-results.md`~~ — не применяются.
  - **Референс:** ограничения — openspec/config.yaml context (RAM ноутбука, p95/recall-гейты).
  - **История ревизий:**
    - Ревизия 1 (2026-08-18, решение человека): протокол ужат для времени прогона на ноутбуке: N 1M → **250K** (1 ГБ f32 — сохраняет RAM-механику, полный 1M-прогон остаётся за change'ом `vectors` по митигации D2), запросы 200 → **100**, efConstruction sweep {100,200} → **{100}**, warmup 20 → 10. Квантизация {f32, i8} и efSearch {50, 200} сохранены — это ключевые параметры решения. Обязателен release-прогон. Протокол 3.2 наследует эти параметры (одна фикстура, те же запросы).
    - Ревизия 2 (2026-08-18, решение человека): **задача ОТМЕНЕНА** — usearch-спайк давал часы прогона и неудовлетворительный результат; вместо самописного решения переходим на **lancedb** как единственного кандидата (задача 3.2). Параметры протокола ревизии 1 переносятся в 3.2.

- [x] 3.2 Spike S3b: измерения lance на синтетической фикстуре 250K×1024 + ADR-решение
  - **Цель:** метрики lance HNSW/IVF при RAM-бюджете ноутбука; запись решения по ANN-движку (единственный кандидат после отмены usearch).
  - **Scope файлов:** `crates/spikes/src/bin/s3b_lance.rs` + зависимость lancedb/lance; затем `docs/adr/0003-ann-engine.md`. Протокол (по ревизии 1 отменённой 3.1): seeded синтетика N=**250K**×1024 f32 (seed зашит, воспроизводимо), M=16, efConstruction {100}, **100** held-out запросов, top-k=50, recall@10 против brute-force ground truth (считается в спайке), p50/p95, peak-RSS-delta при поиске, размер файла на диске, warmup 10. **Прогон ТОЛЬКО в release-сборке** (`cargo run --release -p spikes --bin s3b_lance`) — debug-сборка даёт бессмысленные тайминги и на порядки медленнее. Результаты — в `docs/adr/spike-s3-results.md` (общий appendix).
  - **Зависимости:** 0.2 (БД-фикстура не нужна — design D2). 3.1 ОТМЕНЕНА — lance единственный кандидат, сравнение движков не требуется.
  - **Критерии приёмки:** ADR `docs/adr/0003-ann-engine.md` с решением «движок + конфигурация (M/efConstruction/efSearch/квантизация)» при выполнении p95 < 10 ms, recall@10 ≥ 0.95, RSS-delta ≤ ~2 ГБ; воспроизводимость двух прогонов (<5% разброса); если lance НЕ проходит гейты — ADR помечает NO-GO и задача эскалируется пользователю до модульных change'ов.
  - **Референс:** метрики из docs/adr/spike-s3-results.md; ограничения — openspec/config.yaml context.
  - **История ревизий:**
    - Ревизия 1 (2026-08-18, решение человека): задача становится ЕДИНСТВЕННЫМ спайком ANN-движка — usearch (3.1) отменён (часы прогона, неудовлетворительный результат); протокол ужат: N 1M → 250K, запросы 200 → 100, efConstruction {100,200} → {100}, warmup 20 → 10, обязателен release-прогон. Полный 1M-прогон остаётся за change'ом `vectors` по митигации D2.

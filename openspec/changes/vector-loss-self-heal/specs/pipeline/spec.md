...
- **WHEN** the server starts and some chunk rows have no corresponding vector
- **THEN** a `doc:index` task is enqueued for each affected document, and after the startup drain those chunks have vectors again

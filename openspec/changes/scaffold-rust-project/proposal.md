...
- Четыре контрактные спеки, фиксирующие поведение Go оригинала (`../synopsis`) как есть — источник истины для всех паритетных проверок:
  - `mcp-contract` — 12 MCP tools: имена, JSON-схемы параметров/ответов, транспорт (HTTP SSE), `/health`
...
- `mcp-contract`: внешний контракт MCP API — инструменты, схемы, транспорт; фиксирует поведение Go оригинала как эталон паритета

...
- **Соединение:** пул соединений `r2d2_sqlite` (sync, за `spawn_blocking`; max_size=4) — приложение read-heavy (решение человека 2026-08-20): WAL + несколько соединений → конкурентные чтения, write-транзакции ингеста не блокируют чтения. PRAGMA-parity с оракулом применяется на каждом соединении: WAL, synchronous=NORMAL, cache_size=-64000, mmap_size=268435456, foreign_keys=ON, busy_timeout=5000. `Db::lock()`/`conn()` удалены из публичного API (деадлок-футган), вместо них — `Db::with_conn(f)`; вложенный `exec_tx` → `DbError::NestedTransaction`.

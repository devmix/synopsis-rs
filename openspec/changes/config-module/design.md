...
*Дополнение (ревизия 4, решения человека 2026-08-19):* **формат возвращён к врапперам** — каждая группа повторяющихся элементов в plural-обёртке: `<entities><entity/>`, `<relations><relation/>`, `<sources><source/>`, `<domains><domain/>`, `<expressions><expression/>`, `<attributes><attribute/>`, `<synonyms><synonym/>`, `<methods><method/>` (в `<cross-domain-links>` и `<ner>`), `<regex-rules><regex/>` (в `<extraction>`). **Enum'ы — стандартный serde, без макросов** (tolerant_enum!/strict_enum! не используются в XML): строгие — derive + `#[serde(rename_all = "lowercase")]` без `other` (неизвестное → ошибка парсинга); толерантные — `#[serde(other)] Unknown` (unit) где значение не используется, кастомный `Deserialize`-impl (сохраняет строку) где значение нужно для parity-валидации (`SourceType`: Go проверяет пустоту — `Unknown("bogus")` ≠ пусто). **Маппинг:** публичный API плоский (`Vec`-поля), обёртки — локальные serde-хелперы `deserialize_with` (без дублирующих типов).
...
Цель миграции — функциональная копия, не кодовая: формат конфигов — наш контракт, прикладной код будет скорректирован под него. **Ревизия 4 (финальная):** каждая группа повторяющихся элементов — в plural-обёртке: `<entities><entity/>`, `<relations><relation/>`, `<sources><source/>`, `<domains><domain/>`, `<expressions><expression/>`, `<attributes><attribute/>`, `<synonyms><synonym/>`, `<methods><method/>` (в `<cross-domain-links>` и `<ner>`), `<regex-rules><regex/>` (в `<extraction>`). Контейнеры под-структур: `<cross-domain-links>` (methods/equals/expressions), `<ner>` (methods), `<extraction>` (regex-rules), `<equals>` (min-words). Имена элементов/атрибутов — как в оракуле (global_config.go): `source@path/type/disabled/space/dataset`, `entity@id/name/description`, `attribute@name/type/required/target`, `synonym`, `relation@source/predicate/target/description`, `regex@id/entity/pattern/confidence`, `expression` с `<name>/<description>/<priority>/<where>/<relation-type>`, `min-words`, `llm-confidence-threshold`, `batch-size`. Domain-XML — та же схема (entity/relation/attribute/synonym в обёртках). Следствие: Go-бинарь не парсит новый формат (encoding/xml молча пропускает незнакомые обёртки — потеря данных) — parity по семантике через дифференциальный harness (Go на оригинальном формате, Rust на новом); фикстуры адаптированы с записью исходного sha256 в README.
*Почему:* читаемый структурированный формат (решение человека: теги во врапперах); прямой serde-маппинг без дублирующих типов (DRY/KISS/YAGNI); вложенные теги добавляются там, где нужны.

# entity-extraction Specification (delta)
...
NER-извлечение и разрешение проверяются дифференциально против тестов оракула: regex-правила, валидация ответа, фильтрация порогов, сходство имён (включая кириллицу) дают те же результаты, что зафиксированы в Go-тестах.

#### Scenario: Дифференциальный прогон кейсов оракула
- **WHEN** кейсы из Go-тестов оракула (regex, parse, composite, similarity, resolver) прогоняются через Rust-реализацию
- **THEN** результаты совпадают с зафиксированными в оракуле

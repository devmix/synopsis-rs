...
//! written back.
//!
//! The index is one `DiGraph` plus O(1) lookup `HashMap` indexes:
...
//!
//! ## Fact edges vs entity-link edges (task 1.2)
//!
//! - **Facts are materialized as directed edges.** The builder reads every
//!   fact and adds it as an edge `subject → object` with the fact
//!   `predicate` as the relation type. Nothing is computed at traversal
//!   time — facts become edges at load.
//! - **Entity links are a second edge population**, tagged the same way in
//!   the edge weight ([`EdgeKind::EntityLink`]).
//! - **The traverser distinguishes the two kinds**: fact edges **never**
//!   cross domain boundaries (even with `follow_entity_links=true`); a
//!   cross-domain step is allowed only through an entity-link edge and only
//!   when `follow_entity_links=true` and an entity link exists between the
//!   two nodes.
//!
//! Both populations are edges of the single `DiGraph`, tagged by
//! [`EdgeKind`] in the edge weight. The D4 domain-boundary rule is applied
//! at expansion time by the traverser (task 1.4) using the tag plus node
//! domains. A bidirectional link-existence check is petgraph's
...
//!   attach an edge to a non-existent node.
//! - The name-index key uses the `db` crate's `normalize` (trim + collapse +
//!   lowercase) for BOTH domain and name, consistent with
//!   `EntityDao::get_by_name_fold`.
//! - On a name-key collision (two types share name+domain) the SMALLEST
//!   entity id wins (entities are processed in id order); smallest-id also
//!   matches the cross-domain lookup convention (lowest matching id).
//! - Fact edge `domain`/`metadata` are not carried in the edge weight: the
//!   traverser never reads them (boundary checks use node domains).
//! - The average degree is the standard average TOTAL degree of a directed
//!   graph, `2*E/N`; task 1.5 (metrics.rs) derives the same `2*E/N` directly
//!   from the `DiGraph` edge count.
//! - [`Graph::find_partial`] INCLUDES an exact name match as the top
//!   relevance tier: a query equal to a name must still find it (forcing the
//!   caller to a separate exact lookup would be a footgun for a standalone
//!   API).
//! - [`Graph::find_partial`] results are deterministically sorted (relevance
//!   tier, normalized name, entity id) — never dependent on hash order.
//! - [`Graph::find_exact`] rejects any name that normalizes to empty;
//!   [`Graph::find_partial`] rejects an empty query with
...
/// The population an edge belongs to (see the module docs: fact edges vs
/// entity-link edges).
...
pub struct Graph {
    ...
    /// `"domain:lowercase_name"` → node index; O(1) exact lookup.
    by_name: HashMap<String, NodeIndex>,
    /// Entity type → node indexes in entity-id order.
...
impl Graph {
    ...
    /// a miss — the empty-name guard, generalized to the normalized form.
...
mod tests {
    ...
    // edges: petgraph cannot attach an edge to a non-existent node.
    #[test]
    ...
    /// empty-name guard — a name that normalizes to empty is always a miss.
    #[test]
    ...
    fn partial_finds_prefix_and_substring_sorted() {
        ...
        // IS included (documented design decision: exact matches are the top
        // relevance tier).
        assert_eq!(
    ...
    // an empty domain searches all domains.
    #[test]
    ...
    // Acceptance: an empty query is a caller bug → an explicit error, not an
    // empty result.
    #[test]

...
//! not a code copy).
//!
//! FTS5: `chunks_fts` is an external-content table (`content='chunks'`) kept
//! in sync by the `chunks_fts_ai/ad/au` triggers of the init migration, so
...
const SELECT_CHUNK: &str = "SELECT id, doc_id, chunk_text, sequence_num, start_offset, end_offset, created_at \
     FROM chunks";
...
const FTS_QUERY: &str = "SELECT c.id, c.doc_id, c.chunk_text, c.sequence_num, c.start_offset, c.end_offset, \
...
impl<'conn> ChunkDao<'conn> {
    ...
    pub fn create(
        ...
        self.exec.query_row(
            "INSERT INTO chunks (doc_id, chunk_text, sequence_num, start_offset, end_offset) \
             VALUES (?1, ?2, ?3, ?4, ?5) RETURNING id",
            params![doc_id, chunk_text, sequence_num, start_offset, end_offset],
            |row| row.get(0),
        )
    }
    ...
    /// FTS index via the `chunks_fts_au` trigger. Returns `true` if a row was
    /// updated, `false` if no chunk has `id`.
    pub fn update(
        &self,
        id: i64,
        chunk_text: &str,
        ...
        let changed = self.exec.execute(
            "UPDATE chunks SET chunk_text = ?1, sequence_num = ?2, start_offset = ?3, end_offset = ?4 \
             WHERE id = ?5",
            params![chunk_text, sequence_num, start_offset, end_offset, id],
        )?;
        Ok(changed > 0)
    }
...
mod tests {
    ...
    // update changes text/position/offsets; missing id → false.
    #[test]
    fn update() {
        let db = in_memory_db();
        let doc = seed_doc(&db, "/docs/a.md", None);
        with_chunks(&db, |chunks| {
            ...
            assert!(chunks.update(id, "new text", 5, Some(3), Some(11)).unwrap());
            ...
            assert!(
                !chunks.update(999_999, "x", 0, None, None).unwrap(),
    ...
    fn fts_index_stays_in_sync_with_crud() {
        ...
        let doc = seed_doc(&db, "/docs/a.md", None);
        with_chunks(&db, |chunks| {
            // ai: insert → searchable.
            let id = chunks.create(doc, "zebra stripes", 0, None, None).unwrap();
            assert_eq!(chunks.search_fts("zebra", 20, None).unwrap().len(), 1);

            // au: update → old text drops out, new text enters the index.
            assert!(
                chunks
                    .update(id, "zebra and quokka", 0, None, None)
                    .unwrap()
            );

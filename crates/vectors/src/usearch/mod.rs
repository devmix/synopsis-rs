...
impl UsearchEngine {
    ...
    /// Creates a new engine and attaches the WAL database (the create-side
    /// counterpart of [`Self::open_with_wal`]): the empty layout is created
    /// and a dedicated long-lived `rusqlite::Connection` to `wal_db` (the
    /// knowledge.db path) is attached, so the engine journals its mutations
    /// to the `usearch_vectors_log` table from the first insert (module
    /// docs: WAL journal). `None` (the default) disables the WAL.
    ///
    /// Symmetric with [`Self::open_with_wal`]: the `usearch` tuning section
    /// travels with the index config (ADR 0004 §10) — an absent section
    ...
    pub fn create_with_wal(
        ...
        // (ADR 0004 §10), exactly as in `open_with_wal`.
...
/// Test-only seam (test-hygiene phase-2 task 2.6): the sidecar key
/// manifest at `path` (the [`keys_manifest::read_keys`] codec), exposed
/// so the integration tests can inspect the flushed segment manifests.
pub(crate) fn read_keys(path: &Path) -> Result<Vec<u32>, VectorsError> {
    keys_manifest::read_keys(path)
}

/// Test-only seam (test-hygiene phase-2 task 2.6): the raw RAM layer
/// index size (the C++ `Index` size, before the stale-set subtraction
/// that [`UsearchEngine::count`] applies), so the integration tests can
/// assert on the flush boundary.
pub(crate) fn ram_index_size(engine: &UsearchEngine) -> usize {
    engine.index.size()
}

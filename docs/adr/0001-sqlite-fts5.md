...
Может ли Rust-бинарь владеть полным путём SQLite/FTS5 без CGO-флагов: source-compiled bundled SQLite, инициализация СВЕЖЕЙ базы одним init-миграцией, применение PRAGMA из `../synopsis/configs/config.default.yaml` (включая WAL) и bm25-результаты, идентичные записанным ожиданиям оракула? Какой механизм миграций зафиксировать для боевого crate'а `db`?

## Варианты

1. **rusqlite + libsqlite3-sys `bundled`** — SQLite компилируется из исходников прямо в бинарь; FTS5 входит в каждую bundled-сборку (отдельной фичи больше нет); CGO-флагов нигде нет, и целый класс тихой деградации оракула («нет fts5-модуля → молча склипать») исключён по построению.
...
- Схемный дифф fresh vs fixture (`fixtures/knowledge.db`, v5, provenance и sha256 — fixtures/README.md), Go-artifacts исключены явным списком: **14 таблиц (63 колонки), 25 индексов, 3 триггера** — идентичны.
...
| Запрос | Записанное ожидание оракула | Измерено | Вердикт |
|---|---|---|---|
...
Шов доказан: source-compiled bundled SQLite с FTS5/bm25 полностью работает в Rust без CGO-флагов; схема и ранжирование совпадают с оракулом по записанным ожиданиям. **Вердикт: GO** — crate `db` строится на rusqlite + bundled libsqlite3-sys.
...
- ОДИН init-каталог `migrations/1-init/up.sql` со squashed DDL финального v5-состояния: выведен механически из схемы fixture (sqlite_master + PRAGMA table_info) минус явный список Go-artifacts (`_schema_migrations`, vec0-семейство `chunks_vec*`); семантика кросс-проверена против `../synopsis/migrations/001..005.sql` (read-only, файлы НЕ копируются): 003 исключает `documents.domain` и его индекс; 002 даёт unique-индекс `idx_documents_original_path`; 005 — `app_kv`.
- `PRAGMA user_version` — ЕДИНСТВЕННЫЙ источник истины о состоянии схемы (== 1 после init). Таблица `_schema_migrations` не создаётся и не ведётся; Go-совместимость отслеживания не поддерживается.
...
- `journal_mode=WAL` применяется на bundled SQLite без ошибок и читается обратно как `wal`; появляются sidecar-файлы `-wal`/`-shm` рядом с БД — деплой предполагает запись в каталог данных (как в оракуле).
...
- **Копировать оракульские 001–005 как есть** (отклонено человеком, ревизия 3 задачи 1.1): реплей мёртвой истории на чистой БД бессмыслен; 002/004 — data-миграции, которым применять нечего на пустых таблицах; итоговое состояние полностью описывается одним init-DDL.
- **Bridge/double-write `_schema_migrations` + user_version** (отклонено человеком, design D6): Go-совместимость отслеживания явно не требуется — legacy knowledge.db от Go никогда не открывается, не апгрейдится и не мигрируется (ревизия 2 задачи 1.1: Rust всегда строит БД с нуля).

## Открытые вопросы (закрыты)

- ~~Записанное ожидание `knowledge OR RAG` → **3 хита** противоречит измеренным **18**: в fixture ровно 17 чанков содержат термин «knowledge» и ровно 1 (id 88) — «rag», пересечения нет; при булевой семантике FTS5 (`OR` = объединение) для этого файла результат математически определён — 18.~~ — **Решено решением человека 2026-08-18 (ревизия 4 задачи 1.1):** исходное «3 хита» было артефактом замера с `LIMIT 3` (первые строки: 1, 30, 88); ожидание исправлено на **18** (= объединение 17 + 1, пересечения нет). Выверено двумя независимыми сборками SQLite (bundled 3.53.2 и системный sqlite3 CLI) и код-путём оракула (`../synopsis/internal/database/dao/chunk_dao.go`, SearchFTS передаёт запрос в `chunks_fts MATCH ?` без преобразований); альтернативные интерпретации (AND=0, NEAR=0, wildcard=19, distinct docs=10) значения 3 не дают. Остальные ожидания совпадают с точностью до 4 знаков bm25-score — записаны именно из этого fixture-файла. Спайк s1_sqlite теперь завершается кодом 0 на всех чеках.
